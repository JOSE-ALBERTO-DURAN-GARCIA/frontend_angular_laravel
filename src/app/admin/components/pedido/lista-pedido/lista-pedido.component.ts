...
export class ListaPedidoComponent {
  ...
  generarPDF(recibo){
    ...
    //detalle del pedido
    doc.setFontSize(12);
    doc.text('Codigo de pedido: ' + recibo.cod_pedido, 10, 30);
    doc.text('Fecha: ' + recibo.fecha, 10, 40);
    //agrega mas detalle segun sea necesario
    //Tabla de productos
    const headers = [['Productos', 'Precio', 'Cantidad', 'Subtotal']];
    const productos = recibo.productos.map(producto => {
       const subtotal = parseFloat(producto.precio) * producto.pivot.cantidad;
       return [producto.nombre, `$${producto.precio}`, producto.pivot.cantidad, `$${subtotal.toFixed(2)}`];
    });
    ...
    // Resumen del pedido
    const subtotal = productos.reduce((acc, curr) => acc + parseFloat(curr[3].replace('$', '')), 0);
